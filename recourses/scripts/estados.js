	...
	var servicos = function() {
        ...
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "bStateSave": true,
				"ajax": {
                    "url": baseurl+'estados/get_estados_table/', // ajax source
                    "type": 'GET',
                  /* "data": function ( d ) {
                        return $.extend( {}, d, {
				...
				"drawCallback": function( settings ) {
					$(".remove_estado").confirmation({singleton: true, popout: true});
        ...
        form1.validate({
            ...
            submitHandler:function(form){
                ...
                $.ajax({
                    ...
                    url: baseurl+'estados/novo_estado/',
                    type: "POST",
...
function visivel_estado(estado, elemento, id){
	...
	$.ajax({
		...
		url: baseurl+'estados/muda_estado/?id_estado='+id+'&estado='+estado,
		type: "GET",
		//data: new FormData(form),
    	contentType: false,
        cache: false,
		...
		success: function(response) {
			...
			if(obj){
				...
				if(estado==1){
					...
					campo='<a class="btn btn-sm green" onclick="visivel_estado(0, this, '+id+');"><i class="fa fa-eye"></i> Ativo</a>';
					
				}else{
					
					campo='<a class="btn btn-sm yellow" onclick="visivel_estado(1, this, '+id+');"><i class="fa fa-eye-slash"></i> Inativo</a>';
            ...
            form1.validate({
                ...
                submitHandler:function(form){
					...
					$.ajax({
						...
						url: baseurl+'estados/edita_estado/?id='+id_estado,
...
var ComponentsBootstrapSelect = function () {
    ...
    var UIConfirmations = function () {
        ...
        var handleSample = function () {
            $('.remove_estado').on('confirmed.bs.confirmation', function (e) {
              
               var id = $(this).attr('estado');
               tipo=$(this).parents('tr').find('td:eq(0)').html();
              
               $.ajax({
                    url: baseurl+'estados/remove_estado',
                    type: 'GET',
                    data: 'id='+id,
                    success: function(data) {
                     //alert('ENTROU');
                        ...
                        $('#estado_apagado').html('<b>'+tipo+'</b>');
                        esconde($('.alert_apaga_estado'));
                        
                        $('.alert_estado_removido').show();
                        grid.getDataTable().ajax.reload();
                        if (timer) {
                        ...
                        timer = setTimeout(function(){esconde($('.alert_estado_removido'))}, 8000); 
            ...
            $(".remove_estado").on('show.bs.confirmation', function () {
              ...
              $('#estado_apagar').html('<b>'+tipo+'</b>');
              
              $(".alert_apaga_estado").show();
              if (timer) {
                  clearTimeout(timer); //cancel the previous timer.
                  timer = null;
              }
              timer = setTimeout(function(){esconde($('.alert_apaga_estado'))}, 8000);
              
              App.scrollTo($(".alert_apaga_estado"), 0);
            
                  
  
  
          });   
          $(".remove_estado").on('hide.bs.confirmation', function () {
